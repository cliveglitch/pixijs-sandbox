...
DB_CONTAINER_NAME="pixijs-sandbox-postgres"
...
docker run --name $DB_CONTAINER_NAME -e POSTGRES_PASSWORD=$DB_PASSWORD -e POSTGRES_DB=pixijs-sandbox -d -p 5432:5432 docker.io/postgres
